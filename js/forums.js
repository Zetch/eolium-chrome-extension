...
browser.storage.local
  ...
  .then(preferences => {
    // Get topic list
    const topics = Array.prototype.slice.call(
      document.querySelectorAll('.row.topic:not(.announcement):not(.wikir)')
    )
    ...
    ;(ignored => {
      ...
      const forumName = Array.prototype.slice
        .call(document.querySelectorAll('#breadcrumbs a'))
      ...
      function toggleIgnored (event) {
        ...
        const isIgnored = event.target.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.firstElementChild.classList // .toggle // .title // .row // .col // .row // .col
          .toggle('rowignore')
        ...
        // Save
        browser.storage.local.set({ eolium_forums_ignoredThreads: ignored })
      }
    ...
    function setFilter (event) {
      ...
      topics.forEach(topic => {
        const title = sanitizeText(
          topic.querySelector('.title a').getAttribute('title')
        )
    ...
    function toggleVisibility (event) {
      ...
      browser.storage.local.set({ [optionName]: state })
